...
def load_data(args, dataset_name):
    ...
    elif dataset_name == "fashion_mnist":  # 新增
...
def create_balanced_test_dataset(args):
    ...
    elif args.dataset == "fashion_mnist":  # 新增
        transform_test = transforms.Compose([
...
def evaluate_dtfl_generalization(args, iid_test_loader, net_glob_client_tier, net_glob_server_tier, device):
    ...
    # 对每个tier级别进行评估
    for tier in range(1, 8):  # 使用7个tier级别
...
def evaluate_split_models(net_glob_client_tier, net_glob_server_tier, client_tier, dataset_test, device, num_users):
    """评估所有客户端-服务器split模型在本地测试集的平均准确率"""
    print("\n===== 评估Split模型性能 =====")
        ...
        with torch.no_grad():
            for data, target in test_loader:
                ...
                try:
                    ...
                    logging.error(f"评估客户端 {client_idx} split模型时出错: {str(e)}")
                    continue
        
        if total > 0:
            accuracy = 100.0 * correct / total
            all_accuracies.append(accuracy)
            print(f"客户端 {client_idx} (Tier {tier}) Split模型本地测试准确率: {accuracy:.2f}%")
    
    # 计算平均准确率
    if all_accuracies:
        avg_accuracy = sum(all_accuracies) / len(all_accuracies)
        print(f"所有客户端Split模型平均本地测试准确率: {avg_accuracy:.2f}%")
        return avg_accuracy
    else:
        print("没有成功评估任何客户端Split模型")
        return 0.0
...
for iter in range(epochs):
    ...
    if dtfl_generalization_results:
        ...
        # 记录到wandb
        wandb.log({
            "split/avg_test_accuracy": split_avg_accuracy,
            "epoch": iter
        }, commit=False)
