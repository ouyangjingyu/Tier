...
class DataDistributionVisualizer:
    ...
    # 在 DataDistributionVisualizer 类中添加以下方法
    
    def visualize_clustering_comparison(self, distribution_matrix, 
                                       original_assignments, 
                                       softlabel_assignments,
                                       dataset_name,
                                       comparison_metrics):
        """
        可视化两种聚类方案的对比
        ...
        Args:
            distribution_matrix: 客户端数据分布矩阵 [n_clients, n_classes]
            original_assignments: 原方案聚类结果 {client_id: cluster_id}
            softlabel_assignments: 新方案聚类结果 {client_id: cluster_id}
            dataset_name: 数据集名称
            comparison_metrics: 对比指标 {'NMI': x, 'ARI': y, 'Accuracy': z}
        """
        logging.info(f"生成{dataset_name}聚类方案对比可视化...")
        ...
        n_classes = distribution_matrix.shape[1]
        
        # 1. 原方案聚类热力图（左上）
        ax1 = fig.add_subplot(gs[0, 0])
        self._plot_single_clustering_heatmap(
            ax1, distribution_matrix, original_assignments, 
            "Original Method (Data Distribution)", client_ids, n_classes
        )
        
        # 2. 新方案聚类热力图（右上）
        ax2 = fig.add_subplot(gs[0, 1])
        self._plot_single_clustering_heatmap(
            ax2, distribution_matrix, softlabel_assignments,
            "New Method (Soft Label)", client_ids, n_classes
        )
        
        # 3. 聚类一致性混淆矩阵（中上）
        ax3 = fig.add_subplot(gs[0, 2])
        self._plot_clustering_confusion_matrix(
            ax3, original_assignments, softlabel_assignments, client_ids
        )
        
        # 4. 客户端分组对比（左中）
        ax4 = fig.add_subplot(gs[1, :2])
        self._plot_client_group_comparison(
            ax4, original_assignments, softlabel_assignments, client_ids
        )
        
        # 5. 评估指标展示（右中）
        ax5 = fig.add_subplot(gs[1, 2])
        self._plot_metrics_comparison(ax5, comparison_metrics)
        
        # 6. 组内客户端数据分布对比（底部）
        ax6 = fig.add_subplot(gs[2, :])
        self._plot_group_distribution_comparison(
            ax6, distribution_matrix, original_assignments, 
            softlabel_assignments, client_ids, n_classes
        )
        
        # 总标题
        fig.suptitle(
            f'{dataset_name} - Clustering Methods Comparison\n'
            f'NMI: {comparison_metrics["NMI"]:.3f} | '
            f'ARI: {comparison_metrics["ARI"]:.3f} | '
            f'Accuracy: {comparison_metrics["Accuracy"]:.2%}',
            fontsize=18, fontweight='bold'
        )
        
        # 保存
        save_path = os.path.join(self.save_dir, 
                                f'{dataset_name.lower()}_clustering_comparison.png')
        plt.savefig(save_path, dpi=300, bbox_inches='tight')
        plt.close()
        
        logging.info(f"聚类对比图已保存至: {save_path}")
    
    def _plot_single_clustering_heatmap(self, ax, distribution_matrix, 
                                       assignments, title, client_ids, n_classes):
        """绘制单个聚类方案的热力图"""
        # 按聚类重新排序
        sorted_indices = sorted(range(len(client_ids)), 
                              key=lambda x: (assignments[client_ids[x]], x))
        sorted_matrix = distribution_matrix[sorted_indices]
        
        # 绘制热力图
        im = ax.imshow(sorted_matrix, cmap='YlOrRd', aspect='auto', vmin=0, vmax=1)
        
        # 设置标签
        ax.set_title(title, fontsize=12, fontweight='bold')
        ax.set_xlabel('Classes', fontsize=10)
        ax.set_ylabel('Clients (Sorted by Cluster)', fontsize=10)
        ...
        # Y轴标签
        y_labels = [f'Client {client_ids[i]}\n(G{assignments[client_ids[i]]})' 
                   for i in sorted_indices]
        ax.set_yticks(range(len(client_ids)))
        ax.set_yticklabels(y_labels, fontsize=8)
        
        # 添加聚类分隔线
        current_cluster = assignments[client_ids[sorted_indices[0]]]
        for i, idx in enumerate(sorted_indices[1:], 1):
            if assignments[client_ids[idx]] != current_cluster:
                ax.axhline(y=i-0.5, color='blue', linewidth=2, linestyle='--')
                current_cluster = assignments[client_ids[idx]]
        
        # 颜色条
        plt.colorbar(im, ax=ax, fraction=0.046, pad=0.04)
    
    def _plot_clustering_confusion_matrix(self, ax, original_assignments, 
                                         softlabel_assignments, client_ids):
        """绘制聚类一致性混淆矩阵"""
        # 构建混淆矩阵
        n_clusters_orig = max(original_assignments.values()) + 1
        n_clusters_soft = max(softlabel_assignments.values()) + 1
        n_clusters = max(n_clusters_orig, n_clusters_soft)
        
        confusion = np.zeros((n_clusters, n_clusters), dtype=int)
        
        for cid in client_ids:
            orig_cluster = original_assignments[cid]
            soft_cluster = softlabel_assignments[cid]
            confusion[orig_cluster, soft_cluster] += 1
        
        # 绘制混淆矩阵
        im = ax.imshow(confusion, cmap='Blues', aspect='auto')
        
        ax.set_title('Clustering Consistency Matrix', fontsize=12, fontweight='bold')
        ax.set_xlabel('Soft Label Clusters', fontsize=10)
        ax.set_ylabel('Original Clusters', fontsize=10)
        
        # 设置刻度
        ax.set_xticks(range(n_clusters))
        ax.set_yticks(range(n_clusters))
        ax.set_xticklabels([f'S{i}' for i in range(n_clusters)])
        ax.set_yticklabels([f'O{i}' for i in range(n_clusters)])
    ...
    def _plot_client_group_comparison(self, ax, original_assignments, 
                                     softlabel_assignments, client_ids):
        """绘制客户端分组对比图"""
        n_clients = len(client_ids)
        
        # 准备数据
        orig_clusters = [original_assignments[cid] for cid in client_ids]
        soft_clusters = [softlabel_assignments[cid] for cid in client_ids]
        
        # 绘制条形图
        x = np.arange(n_clients)
        width = 0.35
        
        bars1 = ax.bar(x - width/2, orig_clusters, width, 
                      label='Original Method', alpha=0.8, color='skyblue')
        bars2 = ax.bar(x + width/2, soft_clusters, width,
                      label='Soft Label Method', alpha=0.8, color='lightcoral')
        
        # 标注不一致的客户端
        for i, (orig, soft) in enumerate(zip(orig_clusters, soft_clusters)):
            if orig != soft:
                ax.plot([i-width/2, i+width/2], [orig, soft], 
                       'r--', linewidth=2, alpha=0.5)
        
        ax.set_title('Client Group Assignment Comparison', 
    ...
    def _plot_metrics_comparison(self, ax, metrics):
        """绘制评估指标"""
        ...
        for i, (name, value) in enumerate(zip(metric_names, metric_values)):
            if name == 'Accuracy':
                ...
                label = f'{value:.3f}'
            ax.text(value + 0.02, i, label, va='center', fontsize=11, fontweight='bold')
        
        ax.set_xlim(0, 1.1)
        ax.set_title('Clustering Quality Metrics', fontsize=12, fontweight='bold')
        ...
        ax.text(0.5, -0.3, 
               ...
               'Accuracy: Best Label Matching',
               transform=ax.transAxes, fontsize=8, 
               verticalalignment='top', bbox=dict(boxstyle='round', 
               facecolor='wheat', alpha=0.3))
    
    def _plot_group_distribution_comparison(self, ax, distribution_matrix,
                                           original_assignments, 
                                           softlabel_assignments,
                                           client_ids, n_classes):
        """绘制各组的平均数据分布对比"""
        # 计算原方案各组的平均分布
        orig_groups = defaultdict(list)
        for cid in client_ids:
            cluster_id = original_assignments[cid]
            orig_groups[cluster_id].append(distribution_matrix[cid])
        
        orig_avg_dists = {}
        for cluster_id, dists in orig_groups.items():
            orig_avg_dists[cluster_id] = np.mean(dists, axis=0)
        
        # 计算新方案各组的平均分布
        soft_groups = defaultdict(list)
        for cid in client_ids:
            cluster_id = softlabel_assignments[cid]
            soft_groups[cluster_id].append(distribution_matrix[cid])
        
        soft_avg_dists = {}
        for cluster_id, dists in soft_groups.items():
            soft_avg_dists[cluster_id] = np.mean(dists, axis=0)
        
        # 绘制对比图
        n_clusters = max(len(orig_avg_dists), len(soft_avg_dists))
        
        x = np.arange(n_classes)
        width = 0.8 / (n_clusters * 2)
        
        for i, cluster_id in enumerate(sorted(orig_avg_dists.keys())):
            offset = i * width * 2 - (n_clusters - 1) * width
            ax.bar(x + offset, orig_avg_dists[cluster_id], width,
                  label=f'Original G{cluster_id}', alpha=0.7)
        
        for i, cluster_id in enumerate(sorted(soft_avg_dists.keys())):
            offset = (i + n_clusters) * width * 2 - (n_clusters * 2 - 1) * width
            ax.bar(x + offset, soft_avg_dists[cluster_id], width,
                  label=f'SoftLabel G{cluster_id}', alpha=0.7, hatch='//')
        
        ax.set_title('Average Data Distribution per Cluster', 
                    fontsize=12, fontweight='bold')
        ax.set_xlabel('Classes', fontsize=10)
        ax.set_ylabel('Average Proportion', fontsize=10)
        ax.set_xticks(x)
        ax.set_xticklabels([f'C{i}' for i in range(n_classes)])
        ax.legend(ncol=n_clusters, fontsize=8, loc='upper right')
