...
class KnowledgeDistillationAggregator:
    ...
    def _second_level_distillation(self, cluster_shared_models, cluster_server_models):
        ...
        # 新增：提取共享层的缓冲区（BatchNorm的running_mean/var）
        for name, buffer in student_client_model.named_buffers():
            if 'shared_base' in name:
                global_shared_layers[name] = buffer.clone().cpu()

        global_server_model = {}
        for name, param in student_server_model.named_parameters():
            global_server_model[name] = param.data.clone().cpu()
        # 新增：提取服务器模型的缓冲区
        for name, buffer in student_server_model.named_buffers():
            global_server_model[name] = buffer.clone().cpu()
