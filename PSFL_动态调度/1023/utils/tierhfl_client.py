...
class TierHFLClient:
    ...
    def __init__(self, client_id, tier, train_data, test_data, device='cuda', 
                 lr=0.001, local_epochs=1, device_tier=None, compute_power=None, network_speed=None, storage_capacity=None):
...
class TierHFLClientManager:
    ...
    # 在客户端管理器中添加监控方法
    def add_client(self, client_id, tier, train_data, test_data, device=None, lr=0.001, local_epochs=1, device_tier=None, compute_power=None, network_speed=None, storage_capacity=None):
        """添加客户端"""
    ...
    def update_client_resources(self, client_id, device_tier=None, compute_power=None, network_speed=None, storage_capacity=None):
        if client_id not in self.clients:
            return False
        client = self.clients[client_id]
        if device_tier is not None:
            client.device_tier = device_tier
        if compute_power is not None:
            client.compute_power = compute_power
        if network_speed is not None:
            client.network_speed = network_speed
        if storage_capacity is not None:
            client.storage_capacity = storage_capacity
        return True
