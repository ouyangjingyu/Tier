...
class DistillationDataManager:
    ...
    def __init__(self, dataset_name, data_dir, distillation_ratio=0.15, batch_size=128):
        """
        Args:
            dataset_name: 数据集名称 (cifar10, cifar100, fashion_mnist)
            data_dir: 数据目录
    ...
    def create_distillation_dataset(self, avoid_test_indices=None):
        ...
        if self.dataset_name == "cifar10":
            ...
            return self._create_cifar100_distillation_dataset(avoid_test_indices)
        elif self.dataset_name == "svhn":
            return self._create_svhn_distillation_dataset(avoid_test_indices)
    ...
    def _create_cifar100_distillation_dataset(self, avoid_test_indices):
        """创建CIFAR-100蒸馏数据集"""
        from api.data_preprocessing.cifar100.datasets import CIFAR100_truncated
        
        # 数据预处理 - 使用与 data_loader.py 相同的归一化参数
        transform = transforms.Compose([
            transforms.ToTensor(),
        ...
        # 加载完整训练集
        full_dataset = CIFAR100_truncated(self.data_dir, train=True, download=True, transform=transform)
        
        return self._create_balanced_distillation_split(full_dataset, avoid_test_indices, num_classes=100)
    ...
    def _create_svhn_distillation_dataset(self, avoid_test_indices):
        ...
        full_dataset = SVHN_truncated(self.data_dir, train=True, download=True, transform=transform)
        return self._create_balanced_distillation_split(full_dataset, avoid_test_indices, num_classes=10)
