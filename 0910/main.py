...
def evaluate_global_model(sample_client_model, global_server_model, global_test_loader, device):
    """评估全局模型在全局测试集上的性能"""
    # 确保所有模型都在正确的设备上
    ...
    with torch.no_grad():
        for data, target in global_test_loader:
            ...
            try:
                # 完整的前向传播
                _, shared_features, _ = sample_client_model(data)
                logits = global_server_model(shared_features)
...
def main():
    ...
    for round_idx in range(args.rounds):
        ...
        # 评估全局模型性能
        sample_client_id = 0  # 使用第一个客户端作为样本
        global_model_accuracy = evaluate_global_model(
            client_models[sample_client_id], 
            server_models[sample_client_id],  # 使用聚合后的服务器模型
            global_test_loader, 
            device
        )
