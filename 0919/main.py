...
# 导入数据加载和处理模块
from api.data_preprocessing.cifar10.data_loader import load_partition_data_cifar10, load_partition_data_cifar10_with_clustering
from api.data_preprocessing.cifar100.data_loader import load_partition_data_cifar100
from api.data_preprocessing.cinic10.data_loader import load_partition_data_cinic10
from api.data_preprocessing.fashion_mnist.data_loader import load_partition_data_fashion_mnist, load_partition_data_fashion_mnist_with_clustering
...
def parse_arguments():
    ...
    # 新增聚合策略参数
    parser.add_argument('--aggregation_strategy', type=str, default='knowledge_distillation',
                       choices=['traditional', 'hierarchical', 'knowledge_distillation'],
                       help='聚合策略选择')
    parser.add_argument('--enable_clustering', action='store_true', default=True, 
                       help='是否启用客户端聚类（默认启用）')
    parser.add_argument('--num_clusters', type=int, default=3, 
                       help='客户端聚类数量')
    parser.add_argument('--clustering_method', type=str, default='cosine_similarity',
                       choices=['cosine_similarity', 'simple_grouping'],
                       help='聚类方法')
    parser.add_argument('--cluster_alpha', type=float, default=0.6,
                       help='二级聚合中覆盖度权重和一致性权重的平衡因子')
    ...
    parser.add_argument('--distillation_beta', type=float, default=0.7,
                       help='蒸馏损失权重')
    parser.add_argument('--distillation_epochs', type=int, default=8,
                       help='蒸馏训练轮数')
...
def setup_logging(args):
    ...
    try:
        wandb.init(
            mode="offline",
            project="PSFL_Aggregation_Comparison",
            name=f"{args.running_name}_{args.aggregation_strategy}",
            config=args,
            tags=[f"model_{args.model}", f"dataset_{args.dataset}", 
                  f"clients_{args.client_number}", f"partition_{args.partition_method}",
                  f"aggregation_{args.aggregation_strategy}"],
            group=f"{args.model}_{args.dataset}_{args.aggregation_strategy}"
        )
        ...
        wandb.define_metric("aggregation/*", step_metric="round")
...
def load_dataset(args):
    # 根据聚合策略决定是否启用聚类
    use_clustering = args.enable_clustering and args.aggregation_strategy in ['hierarchical', 'knowledge_distillation']
    
    # 根据数据集类型和是否启用聚类选择相应的数据加载器
    if args.dataset == "cifar10":
        if use_clustering:
            data_loader = load_partition_data_cifar10_with_clustering
        else:
        ...
        if use_clustering:
            data_loader = load_partition_data_fashion_mnist_with_clustering
        else:
    ...
    elif (args.dataset == "fashion_mnist" or args.dataset == "cifar10") and use_clustering:
        # Fashion-MNIST和CIFAR-10聚类版本
        train_data_num, test_data_num, train_data_global, test_data_global, \
        train_data_local_num_dict, train_data_local_dict, test_data_local_dict, \
        class_num, cluster_manager = data_loader(args.dataset, args.data_dir, args.partition_method,
                                args.partition_alpha, args.client_number, args.batch_size,
                                args.num_clusters, args.clustering_method)
        
        dataset = [train_data_num, test_data_num, train_data_global, test_data_global,
                   train_data_local_num_dict, train_data_local_dict, test_data_local_dict, class_num, cluster_manager]
    else:
        # 其他数据集或非聚类版本
        train_data_num, test_data_num, train_data_global, test_data_global, \
        train_data_local_num_dict, train_data_local_dict, test_data_local_dict, \
...
def create_aggregator(args, client_data_sizes, cluster_manager, device):
    ...
    if args.aggregation_strategy == 'traditional':
        # 传统聚合器（不分组）
        logging.info("创建传统全局聚合器...")
        return SimplifiedGlobalAggregator(
            client_data_sizes=client_data_sizes,
            device=device
        )
    
    elif args.aggregation_strategy == 'hierarchical':
        # 分层聚合器（需要聚类）
        if cluster_manager is None:
            logging.warning("分层聚合需要聚类管理器，回退到传统聚合")
            return SimplifiedGlobalAggregator(
                client_data_sizes=client_data_sizes,
                device=device
            )
        logging.info("创建分层聚合器...")
    ...
    elif args.aggregation_strategy == 'knowledge_distillation':
        # 知识蒸馏聚合器（需要聚类）
        if cluster_manager is None:
            logging.warning("知识蒸馏聚合需要聚类管理器，回退到传统聚合")
            return SimplifiedGlobalAggregator(
                client_data_sizes=client_data_sizes,
                device=device
            )
        ...
        # 默认使用传统聚合器
        logging.warning(f"未知的聚合策略: {args.aggregation_strategy}，使用传统聚合")
        return SimplifiedGlobalAggregator(
            client_data_sizes=client_data_sizes,
            device=device
        )
...
def main():
    ...
    if len(dataset) == 9:  # 包含聚类管理器
        train_data_num, test_data_num, _, _, train_data_local_num_dict, train_data_local_dict, test_data_local_dict, class_num, cluster_manager = dataset
        logger.info(f"启用{args.dataset.upper()}聚类，聚类数: {args.num_clusters}")
    else:
    ...
    for round_idx in range(args.rounds):
        ...
        # 全局聚合
        logger.info(f"执行全局聚合 - 策略: {args.aggregation_strategy}")
        ...
        if is_best:
            ...
            try:
                ...
                }, f"{args.running_name}_{args.aggregation_strategy}_best_model.pth")
        ...
        try:
            metrics = {
                ...
                "aggregation/clustering_enabled": args.enable_clustering,
                "aggregation/num_clusters": args.num_clusters if cluster_manager else 0,
                "aggregation/partition_alpha": args.partition_alpha
            }
            
            # 添加知识蒸馏特有的指标
            if args.aggregation_strategy == 'knowledge_distillation' and hasattr(global_aggregator, 'get_distillation_stats'):
                distill_stats = global_aggregator.get_distillation_stats()
                if distill_stats:
                    metrics.update({
                        "distillation/samples": distill_stats.get('distillation_samples', 0),
                        "distillation/temperature": args.distillation_temperature,
                        "distillation/alpha": args.distillation_alpha,
                        "distillation/beta": args.distillation_beta
                    })
    ...
    logger.info(f"使用聚合策略: {args.aggregation_strategy}")
