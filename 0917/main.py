...
# 修改聚合器导入
from utils.simplified_aggregator import SimplifiedGlobalAggregator, HierarchicalAggregator
...
def load_dataset(args):
    ...
    elif (args.dataset == "fashion_mnist" or args.dataset == "cifar10") and args.enable_clustering:
...
def main():
    ...
    if (args.dataset == "fashion_mnist" or args.dataset == "cifar10") and args.enable_clustering:
        # 聚类版本（支持Fashion-MNIST和CIFAR-10）
    ...
    # 创建聚合器（根据是否启用聚类选择不同的聚合器）
    if args.enable_clustering and cluster_manager is not None:
        logger.info("创建分层聚合器...")
        global_aggregator = HierarchicalAggregator(
            client_data_sizes=train_data_local_num_dict,
            cluster_manager=cluster_manager,
            device=device
        )
        logger.info(f"使用分层聚合，聚类数: {args.num_clusters}, 方法: {args.clustering_method}")
    else:
        logger.info("创建传统全局聚合器...")
    ...
    for round_idx in range(args.rounds):
        ...
        try:
            metrics = {
                ...
                "clustering/enabled": args.enable_clustering,
                "clustering/num_clusters": args.num_clusters if args.enable_clustering else 0
            }
